...
import { createPlayer, retreatFromHex, retreatBaseIds, defaultRetreatBase } from '../logic/player.js';
...
export default function App(){
  ...
  // Ouverture DIFFÉRÉE du combat quand l'Empire attaque : le pion doit finir
  // de glisser vers son hex avant que la modale ne recouvre la carte.
  const empireCombatTimer=useRef(null);
  useEffect(()=>()=>clearTimeout(empireCombatTimer.current),[]);
  // Hex de la rencontre en cours — ancre de TOUTES ses récompenses. Le héros
  // s'y trouve pour six factions sur sept ; l'Internationale Noire n'en a pas
  // (`hero: null`), et ses gains se posaient alors sur la clé "null".
  const encHexRef=useRef(null);
  ...
  // Choix de la planque de repli après une défaite (Internationale Noire,
  // quatre bases) : {fromHex, baseId, unitIds}
  const[retreatChoice,setRetreatChoice]=useState(null);
  ...
  const startGame=useCallback((ch=null,matOverride=null)=>{
    ...
    setCombat(null);setEncounter(null);setEndOfTurn(false);setAfterMoveCol(null);setRetreatChoice(null);
    clearTimeout(empireCombatTimer.current);
  ...
  // Hex du drapeau UNIQUE d'une faction (null pour l'Internationale Noire, qui
  // en a quatre — voir `retreatBaseFor` juste en dessous).
  const baseHexIdOf=useCallback((fac)=>homeBaseHex(fac)?.id??null,[]);
  // Base de repli d'une défaite : l'unique drapeau pour six factions, la
  // planque la plus proche pour l'Internationale Noire — que le joueur peut
  // ensuite déplacer vers l'une des trois autres (modale `retreatChoice`).
  const retreatBaseFor=useCallback((fac,fromHex)=>defaultRetreatBase(fac,fromHex),[]);
    ...
    if(cp>=players.length){
      ...
      if(empireIds.length>0){
        ...
        if(pool.length>0){
          ...
          // ── Contact : combat puis dispersion, sur UN état construit à part ──
          // Tout se calcule ici, hors updater : la passe de dispersion doit
          // voir le résultat du combat, et les lignes de journal ne doivent
          // pas naître dans un `setPlayers(prev=>…)` (double-invoke StrictMode).
          const nextPlayers=[...players];
          const empireLogs=[];
          let playersTouched=false;
          // Défenseur = premier joueur ayant une unité COMBATTANTE sur l'hex
          // (une seule bataille par déplacement de patrouille).
          const defIdx=players.findIndex(pl=>combatUnitCount(pl,toId)>0);
          // Patrouille détruite par un bot : plus rien à disperser derrière.
          let patrolKilled=false;
          // Combat interactif à ouvrir APRÈS l'animation du déplacement
          let humanCard=null;
          if(defIdx>=0){
            const pl=players[defIdx];
            if(pl.isBot){
              ...
              let bp={...nextPlayers[defIdx]};
              bp.power-=botSpend;bp.combatCards-=botCC;
              if(botTotal>=card.power){ // defender wins ties (same rule as the human defender)
                empireLogs.push(`⚔🤖 ${bf.name} défend vs ${card.name} (${botTotal} vs ${card.power}) ✅`);
                setEmpire(prev2=>{const n2={...prev2};delete n2[eid];return n2;});
                patrolKilled=true;
                bp.empireKills=(bp.empireKills||0)+1;
                if(bp.empireKills>=3&&!bp.starLiberator){bp.stars++;bp.starLiberator=true;empireLogs.push(`⭐💀 ${bf.name} LIBÉRATEUR !`);}
              } else {
                empireLogs.push(`⚔🤖 ${bf.name} échoue vs ${card.name} (${botTotal} vs ${card.power})`);
                // Défaite : TOUTES les unités du bot présentes décrochent —
                // ses OUVRIERS compris. Ils restaient plantés sous la
                // patrouille victorieuse (partie du 11/08 : « les mechas de
                // l'Empire arrivent sur des ouvriers de bots sans les
                // chasser »), alors que le héros et les mechas décrochaient.
                bp=retreatFromHex(bp,toId,homeBaseHex(bp.faction)?.id??null).player;
              }
              nextPlayers[defIdx]=bp;playersTouched=true;
            } else {
              // Human player — interactive combat (defender), ouvert plus bas
              humanCard=drawEmpireCombat();
            }
          }
          // ── Ouvriers restés sur la case : renvoi à la base (règle Scythe) ──
          ...
          // La passe lit l'état d'APRÈS le combat : les ouvriers d'un bot
          // vaincu ont déjà décroché, ceux d'un TIERS empilé là sont ramassés
          // au passage. L'ancien test lisait `players` avant le combat et
          // sautait la dispersion dès qu'une bataille avait eu lieu.
          // Le défenseur humain est épargné : c'est SON combat qui tranche.
          if(!patrolKilled){
            nextPlayers.forEach((pl,pi)=>{
              if(humanCard&&pi===defIdx)return;
              const hit=pl.workers.filter(w=>w.hexId===toId);
              if(hit.length===0)return;
              const r=retreatFromHex(pl,toId,retreatBaseFor(pl.faction,toId),{units:false});
              empireLogs.push(`🔴👷 L'Empire disperse ${hit.length} ouvrier${hit.length>1?"s":""} de ${FACTIONS[pl.faction].name} sur #${toId} → base`);
              nextPlayers[pi]=r.player;playersTouched=true;
            });
          }
          if(playersTouched)setPlayers(nextPlayers);
          if(empireLogs.length>0)addLogs(empireLogs);
          // ── Le pion glisse AVANT la modale (partie du 11/08) ──
          // La patrouille et le panneau de combat apparaissaient dans la même
          // image : impossible de voir LAQUELLE avait bougé, ni d'où. On
          // laisse courir la transition du pion (0,55 s) avant d'ouvrir.
          if(humanCard){
            addLog(`⚔ L'Empire attaque sur #${toId} ! Force inconnue — entre ${empirePowerRange(1)}`);
            clearTimeout(empireCombatTimer.current);
            empireCombatTimer.current=setTimeout(()=>{
              setCombat({type:"pve",hexId:toId,empireId:eid,empireIds:[eid],empireCard:humanCard,
                phase:"choose",powerSpend:0,cardsSpend:0,empireAttacks:true});
            },700);
          }
        }
      }
    ...
    const timer=setTimeout(()=>{
      ...
      if(empireOnHero&&p.power>=2){
        ...
        if(botTotal>=card.power){
          ...
          Object.assign(p,retreatFromHex(p,botHeroHex,retreatBaseFor(p.faction,botHeroHex)).player);
        }
      }
      ...
      for(let oi=0;oi<n.length;oi++){
        ...
        if(displaced.length>0){
          ...
          let disp={...n[oi]};
          // Chaque hex chassé renvoie vers SA base la plus proche
          dispHexes.forEach(hid=>{disp=retreatFromHex(disp,hid,retreatBaseFor(n[oi].faction,hid),{units:false}).player;});
          n[oi]=disp;
  ...
  const openQueuedEncounter=useCallback((hexId)=>{
    ...
    // Ancre des récompenses : les pickers (bâtiment, ressources, rail) se
    // rouvrent APRÈS `setEncounter(null)` et n'ont plus la carte sous la main.
    encHexRef.current=hexId;
  ...
  const handleHexClick=useCallback((hexId,transportOverride)=>{
    ...
    if(moveSource&&validMoves.has(hexId)){
      ...
      if(movingCombat2){
        ...
        for(let pi=1;pi<players.length;pi++){
          ...
          if(enemyWorkersHere.length>0){
            const ehbId=retreatBaseFor(ep.faction,hexId);
      ...
      // Le garde-fou « une seule rencontre par tour » (fiche §10.1) est LEVÉ
      // (partie du 11/08) : atteindre deux jetons dans le même tour demandait
      // deux déplacements, et n'en résoudre qu'un laissait le second jeton
      // sur la carte sans le dire. Le jeu original ne plafonne rien — il n'y
      // a qu'un héros pour les déclencher ; ici, les quatre groupes
      // d'ouvriers les jouent l'un APRÈS l'autre, dans la file de fin de
      // déplacement (combats d'abord, puis rencontres).
  ...
  // ── Repli après défaite : la PLANQUE se choisit ────────────────────────
  // Une faction se replie sur SA base ; l'Internationale Noire en a quatre,
  // donc un choix (11/08 : « ils se replient sur une base au choix comme pour
  // les autres factions, sauf que les autres n'en ont qu'une »). Les pions
  // sont posés d'office sur la plus proche — il faut bien qu'ils soient
  // quelque part — puis la modale permet de les envoyer sur une autre.
  const retreatLabel=useCallback((fac,baseId)=>{
    if(baseId==null)return "Repli hors carte.";
    const idx=retreatBaseIds(fac).indexOf(baseId);
    return retreatBaseIds(fac).length>1
      ? `Repli sur la planque ${idx+1} (sortie #${(ADJ[baseId]||[]).find(h=>!hMap[h]?.base)}).`
      : "Retraite vers la base.";
  },[]);

  // Ouvre le choix de planque quand la faction en a plusieurs ET qu'il y a
  // bien eu du monde à replier. Appelée APRÈS l'updater, avec l'état d'AVANT.
  const offerRetreatChoice=useCallback((hexId,baseId)=>{
    if(!me||baseId==null||retreatBaseIds(me.faction).length<2)return;
    const unitIds=[...(me.workers||[]),...(me.mechs||[])].filter(u=>u.hexId===hexId).map(u=>u.id);
    if(unitIds.length===0)return;
    setRetreatChoice({fromHex:hexId,baseId,unitIds});
  },[me]);

  // Déplace le groupe replié vers une autre planque (gratuit : c'est le même
  // repli, pas un déplacement — le choix aurait dû être fait à la défaite).
  const pickRetreatBase=useCallback((baseId)=>{
    setRetreatChoice(rc=>{
      if(!rc)return null;
      if(baseId!==rc.baseId){
        const ids=new Set(rc.unitIds);
        setPlayers(prev=>{
          const n=[...prev];
          n[0]={...n[0],
            workers:n[0].workers.map(w=>ids.has(w.id)?{...w,hexId:baseId}:w),
            mechs:n[0].mechs.map(m=>ids.has(m.id)?{...m,hexId:baseId}:m)};
          return n;
        });
        const idx=retreatBaseIds(players[0]?.faction).indexOf(baseId);
        addLog(`🏴 Le réseau redirige ${rc.unitIds.length} unité${rc.unitIds.length>1?"s":""} vers la planque ${idx+1}`);
      }
      return null;
    });
  },[players,addLog]);
  ...
  const resolveCombat=useCallback(()=>{
    ...
    if(combat.type==="pvp_defense"){
      ...
      const myBaseId=retreatBaseFor(me.faction,combat.hexId);
      const atkBaseId=retreatBaseFor(attacker.faction,combat.hexId);
      setPlayers(prev=>{
        const n=[...prev];
        ...
        if(win){
          ...
          n[atkIdx]=retreatFromHex(n[atkIdx],combat.hexId,atkBaseId).player;
          ...
          n[0]=retreatFromHex(n[0],combat.hexId,myBaseId).player;
      ...
      addLog(win?`🛡 Vous repoussez ${af.name} ! ⭐ Étoile de combat.`:`❌ ${af.name} prend #${combat.hexId}... ${retreatLabel(me.faction,myBaseId)}`);
      if(!win)offerRetreatChoice(combat.hexId,myBaseId);
      setCombat(null);
    ...
    if(combat.type==="pve"){
      ...
      setPlayers(prev=>{
        ...
        if(!win){
          ...
          Object.assign(p,retreatFromHex(p,combat.hexId,hbIdPve).player);
        }
        // Le déplacement a déjà été compté par le mouvement lui-même
        n[0]=p;return n;
      });

      if(!win)offerRetreatChoice(combat.hexId,hbIdPve);
    ...
    if(combat.type==="pvp"){
      ...
      if(enemy.faction==="acadiane"&&(enemy.unlockedAbilities||[]).includes(2)&&Math.random()<0.5){
        ...
        const ehbIdWF=retreatBaseFor(enemy.faction,combat.hexId);
        setPlayers(prev=>{
          const n=[...prev];
          ...
          n[combat.enemyIdx]=retreatFromHex(n[combat.enemyIdx],combat.hexId,ehbIdWF).player;
      ...
      const hbId2=retreatBaseFor(me.faction,combat.hexId);
      const ehbId2=retreatBaseFor(enemy.faction,combat.hexId);
      ...
      setPlayers(prev=>{
        ...
        if(win){
          ...
          n[combat.enemyIdx]=retreatFromHex(n[combat.enemyIdx],combat.hexId,ehbId2).player;
          ...
          const rr=retreatFromHex(n[0],combat.hexId,hbId2);
          n[0]=rr.player;
      ...
      if(win){
        ...
        addLog(`❌ Défaite PvP... ${retreatLabel(me.faction,hbId2)}`);
        offerRetreatChoice(combat.hexId,hbId2);
  ...
  },[combat,me,players,empire,myFaction,myMat,addLog,effMoveLimit,endMoveDone,offerRetreatChoice,retreatLabel,retreatBaseFor]);
  ...
  const resolveWhiteFlag=useCallback(()=>{
    ...
    offerRetreatChoice(combat.hexId,myBaseId);
    setCombat(null);
    setCurrentP(combat.resumeCp);setBotRunning(true);
  },[combat,me,players,addLog,offerRetreatChoice,retreatBaseFor]);
  ...
  const resolveEncounter=useCallback((choiceIdx)=>{
    ...
    setPlayers(prev=>{
      ...
      // Les gains atterrissent sur le hex de LA RENCONTRE : le héros y est,
      // sauf pour l'Internationale Noire qui n'en a pas (data/encounters.js,
      // `encAnchor`). Sans cette ancre, ses gains partaient sur la clé "null".
      p.encHex=encounter.hexId;
      choice.effect(p);
      delete p.encHex;
    ...
    // de la RENCONTRE — même flux `railPlacement` que le Build/l'usine (source
    // dédiée "encounter" pour reprendre le tour normalement, pas comme une
    // action bas).
    const encHex=encHexRef.current;
    if(choice.grantsGare){
      if((me.buildings||[]).length>=4||(me.buildings||[]).some(b=>b.hexId===encHex)){addLog(`⚠ Pas de place pour la Gare (rencontre)`);resumeAfterEncounter();return;}
      setPlayers(prev=>{
        const n=[...prev];const p={...n[0],buildings:[...(n[0].buildings||[]),{type:"gare",hexId:encHex}]};
        const earned=p.buildings.length>=4&&!p.starBuildings;
        if(earned){p.stars++;p.starBuildings=true;}
        n[0]=p;return n;
      });
      addLog(`🚂 Gare édifiée sur #${encHex} (rencontre) — posez 3 segments de rail`);
      setRailPlacement({remaining:3,total:3,fromHex:null,gareHex:encHex,source:"encounter"});
      return;
    }
    if(choice.railSegments>0){
      addLog(`🛤 Pose de ${choice.railSegments} segments de rail depuis #${encHex} (rencontre)`);
      setRailPlacement({remaining:choice.railSegments,total:choice.railSegments,fromHex:encHex,gareHex:null,source:"encounter"});
      return;
    }
  ...
  // ── RÉCOMPENSE RENCONTRE : ressources AU CHOIX (posées sur le hex atteint) ──
  const doEncounterResource=useCallback((resType)=>{
    ...
    setPlayers(prev=>{
      const n=[...prev];const p={...n[0],resources:{...n[0].resources}};
      Object.keys(n[0].resources).forEach(k=>{p.resources[k]={...n[0].resources[k]};});
      const key=String(encHex);
      if(!p.resources[key])p.resources[key]={};
      p.resources[key][resType]=(p.resources[key][resType]||0)+1;
      n[0]=p;return n;
    });
    addLog(`📦 +1 ${resFR(resType)} sur #${encHex} (rencontre)`);
  ...
  // ── RÉCOMPENSE RENCONTRE : bâtiment gratuit (posé sur le hex atteint) ──
  const doEncounterBuild=useCallback((buildingType)=>{
    if(!me)return;
    const hex=encHexRef.current;
  ...
  const doProduce=()=>{
    if(!me)return;
    if(!canPayProduce(me)){
      const c=getProduceCost(me.workers.length,myMat);
      const missing=[];
    ...
    const costLabel=produceCostLabel(me.workers.length,myMat);
    // ── v0.16 : updater PUR ────────────────────────────────────────────
    ...
    <div style={{height:"100vh",display:"grid",gridTemplateRows:"var(--top-h) 1fr auto",gridTemplateColumns:"var(--left-w) 1fr var(--right-w)",background:"var(--bg)",color:"var(--text)",overflow:"hidden"}}>
      ...
      <div style={{gridColumn:"1/-1",display:"flex",alignItems:"center",padding:"6px 16px",gap:10,background:"linear-gradient(180deg,#282013,#1c150c)",borderBottom:"1px solid var(--panel-edge)",boxShadow:"inset 0 -1px 0 rgba(216,201,163,0.07)",flexShrink:0,height:"var(--top-h)",overflow:"hidden"}}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:10,marginRight:4,flexShrink:0}}>
          <div style={{width:54,height:54,borderRadius:"50%",background:uiInk(myFaction)+"22",border:`2px solid ${uiInk(myFaction)}`,display:"flex",alignItems:"center",justifyContent:"center",overflow:"hidden",flexShrink:0,boxShadow:`0 0 10px ${uiInk(myFaction)}33`}}>
            <FactionCrest factionId={me.faction} size={46} color={uiInk(myFaction)}/>
          </div>
              ...
              <div key={op.faction} style={{display:"flex",alignItems:"center",gap:10,flexWrap:"wrap",
                ...
                <div style={{display:"flex",alignItems:"center",gap:8,minWidth:150}}>
                  <div style={{width:30,height:30,borderRadius:"50%",background:uiInk(of)+"22",border:`2px solid ${uiInk(of)}`,display:"flex",alignItems:"center",justifyContent:"center",overflow:"hidden",flexShrink:0}}>
                    <FactionCrest factionId={op.faction} size={25} color={uiInk(of)}/>
                  </div>
                  <div style={{lineHeight:1.2}}>
      ...
      <div style={{position:"relative",overflow:"hidden",background:"radial-gradient(ellipse at 50% 45%,#16140e,var(--bg-map))",cursor:isPanning?"grabbing":"grab",touchAction:"none"}}>
        ...
        <svg ref={mapRef} viewBox={`${mapView.x} ${mapView.y} ${mapView.w} ${mapView.h}`} style={{width:"100%",height:"100%",display:"block",minHeight:"100%"}}
          ...
          {HEXES.map(hex=>{
            ...
            // les hex par lesquels ses unités entrent — au premier tour, et
            // après chaque repli.
        ...
        {(combat||encounter||encounterBuild||encounterEnlist||encounterUpgrade||encounterResources||rougeRiver||factoryPreview||abilityOffer||stealOffer||retreatChoice)&&(
          <div style={{position:"absolute",top:0,left:0,right:0,bottom:0,background:"rgba(0,0,0,0.6)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:10}}>
            <div style={{maxWidth:460,width:"92%",maxHeight:"80vh",overflow:"auto",borderRadius:12,border:"1px solid var(--border-light)",boxShadow:"0 10px 50px rgba(0,0,0,0.8)"}}>
              ...
              {/* ═══ REPLI — CHOIX DE LA PLANQUE (Internationale Noire) ═══
                  Une faction se replie sur SA base ; celle-ci en a quatre, donc
                  le repli est un choix. Les pions sont déjà posés sur la plus
                  proche : cliquer une autre planque les y envoie. */}
              {retreatChoice&&(()=>{
                const bases=retreatBaseIds(me.faction);
                const nUnits=retreatChoice.unitIds.length;
                return(
                  <div style={{padding:"20px",background:"linear-gradient(180deg,#150d12,var(--bg2))",borderRadius:10,border:"1px solid #9E3B4E",animation:"slideUp 0.35s ease"}}>
                    <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:12}}>
                      <div style={{width:44,height:44,borderRadius:"50%",background:"rgba(158,59,78,0.18)",display:"flex",alignItems:"center",justifyContent:"center",border:"2px solid #9E3B4E",flexShrink:0}}>
                        <FactionCrest factionId={me.faction} size={30} color={uiInk(myFaction)}/>
                      </div>
                      <div>
                        <div style={{fontFamily:"var(--font-title)",color:"#E08090",fontSize:18,fontWeight:700}}>Repli du réseau</div>
                        <div style={{fontSize:13,color:"var(--text-dim)",marginTop:2}}>
                          #{retreatChoice.fromHex} est perdu — <b>{nUnits} unité{nUnits>1?"s":""}</b> décroche{nUnits>1?"nt":""}. Sur quelle planque ?
                        </div>
                      </div>
                    </div>
                    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
                      {bases.map((bid,i)=>{
                        const exit=(ADJ[bid]||[]).find(h=>!hMap[h]?.base);
                        const here=bid===retreatChoice.baseId;
                        const held=[...(me.workers||[]),...(me.mechs||[])].filter(u=>u.hexId===bid&&!retreatChoice.unitIds.includes(u.id)).length;
                        return(
                          <button key={bid} onClick={()=>pickRetreatBase(bid)} className="act-btn"
                            style={{textAlign:"left",padding:"10px 12px",
                              borderColor:here?"#9E3B4E":"var(--border)",
                              background:here?"rgba(158,59,78,0.16)":"transparent"}}>
                            <div style={{fontWeight:700,color:here?"#E08090":"var(--gold)",fontSize:14}}>Planque {i+1}{here?" ✓":""}</div>
                            <div style={{fontSize:12,color:"var(--text-muted)",marginTop:2}}>
                              sortie #{exit} · {TERRAINS[hMap[exit]?.t]?.name||hMap[exit]?.t}
                              {held>0&&<> · {held} déjà sur place</>}
                            </div>
                          </button>
                        );
                      })}
                    ...
                    <div style={{fontSize:12,color:"var(--text-muted)",marginTop:10,lineHeight:1.5}}>
                      Une unité sur une planque en ressort par sa sortie, au prix d'un déplacement — comme le héros d'une autre faction quitte sa base.
                    </div>
                  </div>
                );
              })()}
      ...
      <div style={{display:"flex",flexDirection:"column",background:"linear-gradient(180deg,#241d12,#171209 60%,#100c07)",borderLeft:"1px solid var(--panel-edge)",boxShadow:"inset 1px 0 0 rgba(216,201,163,0.06)",overflow:"hidden"}}>
        ...
        <div style={{height:74,position:"relative",overflow:"hidden",borderBottom:"1px solid var(--panel-edge)",flexShrink:0}}>
          {/* Sans illustration (Internationale Noire), le noir du drapeau. */}
          {FACTION_ART[me.faction]
            ...
            :<div style={{width:"100%",height:"100%",background:`radial-gradient(120% 100% at 50% 0%, ${uiInk(myFaction)}22 0%, #0b0a08 70%)`}}/>}
          ...
          <div style={{position:"absolute",left:10,bottom:6,display:"flex",alignItems:"center",gap:6}}>
            <FactionCrest factionId={me.faction} size={22} color={uiInk(myFaction)} style={{filter:"drop-shadow(0 1px 4px rgba(0,0,0,0.7))"}}/>
        ...
        {/* Le corps du bloc ne porte QUE la capacité ; les dérogations de
            faction (`rules`) sont repliées derrière un dépliant : elles se
            consultent, elles ne se relisent pas à chaque tour. */}
        {myFaction.ability&&(
          <div style={{padding:"5px 10px",borderBottom:"1px solid var(--border)",flexShrink:0,fontSize:13,display:"flex",gap:6,alignItems:"flex-start",background:"rgba(200,112,64,0.05)"}}>
            <span>🏴</span>
            <div style={{minWidth:0}}>
              ...
              {myFaction.rules?.length>0&&(
                <details style={{marginTop:3}}>
                  <summary style={{cursor:"pointer",fontSize:11.5,color:"var(--gold-dim)",listStyle:"revert"}}>Règles de faction ({myFaction.rules.length})</summary>
                  <ul style={{margin:"3px 0 2px 14px",padding:0,color:"var(--text-muted)",fontSize:11.5,lineHeight:1.5}}>
                    {myFaction.rules.map((r,i)=><li key={i}>{r}</li>)}
                  </ul>
                </details>
              )}
            </div>
                  ...
                  <button onClick={()=>{if(upgradePicking)return;if(!disabled){pushHistory();setPreActionSnapshot({...players[0],workers:[...players[0].workers.map(w=>({...w}))],mechs:[...players[0].mechs.map(m=>({...m}))],buildings:[...(players[0].buildings||[]).map(b=>({...b}))],resources:{...Object.fromEntries(Object.entries(players[0].resources).map(([k,v])=>[k,{...v}]))},movedUnits:[...(players[0].movedUnits||[])]});setSelAction(action);}}}
                    ...
                    <div style={{padding:"7px 10px",display:"flex",alignItems:"center",gap:8}}>
                      <div style={{flex:1,minWidth:0,display:"flex",flexDirection:"column",gap:4}}>
                        ...
                        {action==="Produce"&&<ProduceTrack nWorkers={me.workers.length} mat={mat} size={19}/>}
                      </div>
          ...
          {isMyTurn&&!combat&&!encounter&&!rougeRiver&&selAction&&(
            <div style={{padding:"12px 16px",fontSize:14,animation:"slideUp 0.25s ease"}}>
              ...
              {selAction==="Produce"&&(<div>
                ...
                {(()=>{
                  const nw=me.workers.length;const costStr=produceCostLabel(nw,myMat);const canPay=canPayProduce(me);
                  const maxN=2+topUpgradeCount(me,"Produce","nourriture");
                  const moulinHex=(me.buildings||[]).find(b=>b.type==="moulin")?.hexId;
                  ...
                  return(<div>
                    ...
                    <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:8}}>
                      <ProduceTrack nWorkers={nw} mat={myMat} size={26}/>
                    </div>
