...
const transferHexResources = (loser, winner, hexId) => {
...
export const resolveBotPvp = (playersArr, attIdx, defIdx, hexId) => {
  ...
  // ── Engagements (heuristique) : attaquant plus agressif, défenseur prudent ──
  const attCB = getCombatBonus(att, hexId, true, def.combatCards);
  const defCB = getCombatBonus(def, hexId, false, att.combatCards);
  const attSpend = Math.min(Math.floor(att.power * 0.7) + 1, 7, att.power);
  const defSpend = Math.min(Math.floor(def.power * 0.6), 7, def.power);
  const attCC = Math.min(att.combatCards, unitsOnHex(att, hexId) + attCB.cardBonus);
  const defCC = Math.min(def.combatCards, unitsOnHex(def, hexId) + defCB.cardBonus);
